...
var configSchema = exports_external.object({
  ...
  btcUsdFeedAddress: exports_external.string(),
  demoOverrides: exports_external.array(exports_external.object({ marketId: exports_external.string(), resolved: exports_external.boolean(), yesWon: exports_external.boolean() }))
});
var POSITION_BOOK_ABI = [
...
var LEG_STATUS_OPEN = 0;
async function main() {
  const runner = await Runner.newRunner({ configSchema });
  await runner.run((config) => [
    handler(new CronCapability().trigger({ schedule: config.schedule }), (runtime2) => {
      const caps = {
        evmClient: new ClientCapability(BigInt("10344971235874465080")),
        httpClient: new ClientCapability2
      };
      return settlementWorkflow(runtime2, config, caps);
    })
  ]);
}
function settlementWorkflow(runtime2, config, caps) {
  runtime2.log("Underlay settlement workflow triggered");
  const openPositions = getOpenPositions(runtime2, config, caps);
  if (openPositions.length === 0) {
    runtime2.log("No open positions");
    return "no_positions";
  }
  runtime2.log(`Checking ${openPositions.length} open position(s)`);
  const batch = runtime2.runInNodeMode((nodeRuntime) => collectResolvedLegs(nodeRuntime, config, caps, openPositions), consensusIdenticalAggregation())().result();
  if (batch.length === 0) {
    runtime2.log("No legs ready for resolution");
    return "none_ready";
  }
  runtime2.log(`Submitting ${batch.length} leg resolution(s) onchain`);
  submitResolutions(runtime2, config, caps, batch);
  return `resolved_${batch.length}`;
}
function getOpenPositions(runtime2, config, caps) {
  const callData = encodeFunctionData({ abi: POSITION_BOOK_ABI, functionName: "getOpenPositions" });
  const reply = caps.evmClient.callContract(runtime2, {
    call: encodeCallMsg({ from: ZERO_ADDRESS, to: config.positionBookAddress, data: callData }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
  return decodeFunctionResult({ abi: POSITION_BOOK_ABI, functionName: "getOpenPositions", data: bytesToHex(reply.data) });
}
function getPositionLegs(runtime2, config, caps, positionId) {
  const callData = encodeFunctionData({ abi: POSITION_BOOK_ABI, functionName: "getPositionLegs", args: [positionId] });
  const reply = caps.evmClient.callContract(runtime2, {
    call: encodeCallMsg({ from: ZERO_ADDRESS, to: config.positionBookAddress, data: callData }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
  return decodeFunctionResult({ abi: POSITION_BOOK_ABI, functionName: "getPositionLegs", data: bytesToHex(reply.data) }).map((leg) => ({ ...leg }));
}
function collectResolvedLegs(nodeRuntime, config, caps, positionIds) {
  const resolved = [];
  for (const positionId of positionIds) {
    const legs = getPositionLegs(nodeRuntime, config, caps, positionId);
    for (let i2 = 0;i2 < legs.length; i2++) {
      const leg = legs[i2];
      if (leg.status !== LEG_STATUS_OPEN)
        continue;
      const outcome = checkLegResolution(nodeRuntime, config, caps, leg);
      if (outcome !== null)
        resolved.push({ positionId, legIndex: i2, won: outcome });
    }
  }
  return resolved;
}
function checkLegResolution(nodeRuntime, config, caps, leg) {
...
function checkPolymarket(nodeRuntime, config, caps, conditionId, bettorOutcome) {
  ...
  if (market.question)
    crossReferenceChainlinkPrice(nodeRuntime, config, caps, market.question, yesWon);
  return won;
}
function crossReferenceChainlinkPrice(runtime2, config, caps, question, polymarketYesWon) {
  ...
  const reply = caps.evmClient.callContract(runtime2, {
    call: encodeCallMsg({ from: ZERO_ADDRESS, to: feedAddress, data: callData }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
  const [, answer] = decodeFunctionResult({ abi: PRICE_FEED_ABI, functionName: "latestRoundData", data: bytesToHex(reply.data) });
  ...
  if (thresholdMatch) {
    ...
    if (aboveKeyword || belowKeyword) {
      ...
      runtime2.log(`[Chainlink ✓] ${ticker}/USD = $${price.toFixed(2)} | threshold $${threshold.toFixed(2)} | ${aboveKeyword ? "above" : "below"} → ${chainlinkYesWon} | Polymarket says ${polymarketYesWon} | ${agree ? "AGREE ✓" : "DISAGREE ⚠"}`);
      return;
    }
  }
  runtime2.log(`[Chainlink] ${ticker}/USD = $${price.toFixed(2)} — Polymarket outcome cross-referenced for "${question}"`);
}
function submitResolutions(runtime2, config, caps, batch) {
  const positionIds = batch.map((r) => r.positionId);
  const legIndexes = batch.map((r) => r.legIndex);
  const outcomes = batch.map((r) => r.won);
  const callData = encodeFunctionData({ abi: SETTLEMENT_ABI, functionName: "resolveLegs", args: [positionIds, legIndexes, outcomes] });
  const report2 = runtime2.report(prepareReportRequest(callData, EVM_DEFAULT_REPORT_ENCODER)).result();
  caps.evmClient.writeReport(runtime2, { receiver: config.settlementManagerAddress, report: report2, gasConfig: { gasLimit: "500000" } }).result();
  runtime2.log(`resolveLegs submitted for ${[...new Set(positionIds)].length} position(s), ${batch.length} leg(s)`);
}

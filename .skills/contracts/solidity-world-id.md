...
export function WorldIDButton({
  ...
  // Read the gate from contract config or app config state
  if (stake <= worldIdGate) return null;
...
- Do not hardcode the World ID gate in frontend components. Read it from protocol config.

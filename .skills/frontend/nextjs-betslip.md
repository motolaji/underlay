...
Use when building the cart or betslip interaction components inside the app workspace, such as `components/cart/CartDrawer.tsx`, `components/cart/CartDock.tsx`, or equivalent child components. This is the core bettor interaction surface — where legs combine into a position, odds are shown, risk is scored, and the position is submitted.

## Context

The betslip should behave like a cart, not a permanently fixed sidebar. It opens when the first leg is added, can be closed without losing draft state, and shows all selected legs, combined odds, AI risk score, stake input, World ID gate, and the submission CTA. It calls `/api/risk` with a debounce after leg or stake changes. Read FRONTEND_DESIGN_PROMPT.md before building.
...
```
CartDrawer.tsx               Main container
  CartLineItem.tsx           Individual leg display
...
CartDock.tsx                 Floating desktop dock / mobile cart bar
```
...
export function BetSlip() {
  ...
  const { legs, stake, setStake, setSlip, setScoring, clearSlip } =
    useBetslip();
        ...
        {legs.length === 1 && (
          <div className="px-4 py-3">
            <p className="text-xs font-mono text-text-tertiary">
              Add at least 1 outcome to build a position
            </p>
          </div>
        )}

        {/* Odds + Risk (shown when 1+ legs) */}
        {legs.length >= 1 && (
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <span
            className={`text-base font-mono font-medium text-text-primary ${
              isLoading ? "opacity-50" : ""
            }`}
          >
...
export function SubmitButton({
  ...
  const needsWorldId = stake > config.worldIdGate;
...
- The cart can close without clearing draft state — successful submission can still show a receipt or confirmation state
- Remove button on LegCard is hidden until hover — use group-hover pattern
- Keep the cart closable without clearing draft state
- Leg limits are config-driven — do not hardcode `4`
- Combined odds and payout cap should remain visible even when the cart is compact
- On mobile use a cart sheet, not a fixed panel

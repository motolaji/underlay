...
Use when building `app/app/page.tsx` — the main underwriting workspace that displays Polymarket markets and lets users build a position cart.

## Context

This is the app workspace, not the homepage. It fetches live markets from the Polymarket Gamma API via our Next.js proxy route (`/api/markets`). Markets display in a browser/list layout. Users add legs into a closable cart. The cart should not permanently consume workspace width on desktop and should open as a drawer or sheet when needed. Read FRONTEND_DESIGN_PROMPT.md before building this.
...
- market browser as the primary surface
- floating cart dock
- closable cart drawer on demand

Mobile (<1024px):
- market browser full width
- sticky bottom cart bar
- cart opens as a full-height sheet
```
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const validMarkets = markets.filter(
      (m) =>
        m.outcomePrices && m.active && !m.closed && m.outcomes?.length === 2
    );
...
export function MarketCard({ market }: MarketCardProps) {
  ...
  function handleAdd(outcome: "yes" | "no") {
    ...
    const probability =
      outcome === "yes" ? market.yesProbability : market.noProbability;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {isInSlip ? (
          <span className="text-xs font-mono text-blue-400">✓ In slip</span>
        ) : slipFull ? (
          <span className="text-xs font-mono text-text-tertiary">
            Cart full (10 max)
          </span>
...
- The cart should preserve market browsing context — do not force permanent split layout
- On mobile the cart is a sheet, not a fixed side panel
- Keep leg count config-driven — do not hardcode `4`

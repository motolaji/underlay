...
Chainlink CRE (Runtime Environment) is the orchestration layer for our settlement flow. It runs on a Decentralised Oracle Network (DON) with Byzantine Fault Tolerant consensus. We use the TypeScript SDK compiled to WASM for CRE runtime constraints. The workflow can be simulated locally with `cre workflow simulate` — Chainlink team deploys it to live CRE network during the hackathon if simulation succeeds.
...
Repo-level package management uses npm. CRE runtime constraints are separate from repo package management.

If CRE tooling requires Bun for workflow compilation/runtime, treat Bun as a workflow-specific exception inside `cre-workflow/`, not as the repo-wide package manager.

```bash
cd underlay
mkdir cre-workflow && cd cre-workflow

# Install Bun if required by the CRE toolchain/runtime in your current environment
...
# Install workflow dependencies using the runtime/tooling required by CRE in that environment
```
...
async function verifyCryptoLegWithPriceFeed(
  ...
  if (
    !marketQuestion.toLowerCase().includes("eth") &&
  ...
  evmClient
    .writeContract(runtime, {
      call: encodeFunctionData({
        ...
        args: [resolution.positionId as `0x${string}`, resolution.outcomes],
      }),
      to: config.settlementManagerAddress as `0x${string}`,
    })
    .result();
...
- CRE may require Bun depending on the current toolchain/runtime — treat it as a workflow-specific exception, not the repo-wide package manager

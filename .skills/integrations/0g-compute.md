...
Important: frontend and client-side web3 remain `viem` only. If the 0G SDK requires `ethers`, keep that usage isolated to server-only integration files such as API routes or storage helpers.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.warn(
      "0G Compute unavailable, falling back to rule-based:",
      computeErr
    );
...
async function score0GCompute(input: RiskRequest) {
  ...
  const llmService = services.find(
    (s) => s.serviceType === "llm" && s.model.toLowerCase().includes("llama")
  );
  ...
  await broker.processResponse(llmService.provider, llmService.name, content);
